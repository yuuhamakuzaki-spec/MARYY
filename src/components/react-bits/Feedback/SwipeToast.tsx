    ...
    >
      <div className="min-h-0">
        <div className="opacity-100 group-data-[inline=true]:mt-[var(--st-gap)] [transform:translateY(0)] [transition:transform_var(--st-slide)_cubic-bezier(0.23,1,0.32,1),opacity_calc(var(--st-slide)*0.6)_ease] starting:opacity-0 starting:[transform:translateY(100%)] group-data-[mounted=false]:opacity-0 group-data-[mounted=false]:[transform:translateY(100%)] group-data-[phase=closing]:opacity-0 group-data-[phase=closing]:[transform:translateY(100%)] group-data-[phase=closing]:[transition:transform_calc(var(--st-slide)*0.7)_cubic-bezier(0.23,1,0.32,1),opacity_calc(var(--st-slide)*0.5)_ease] group-data-[phase=gone]:invisible group-data-[phase=gone]:opacity-0 group-data-[phase=gone]:[transform:translateY(100%)] group-data-[instant]:[transition-duration:0s] motion-reduce:[transform:none]! motion-reduce:[transition:opacity_200ms_ease]">
          <motion.div
            ref={cardRef}
            className="relative flex cursor-grab touch-none items-center gap-3 overflow-hidden border border-white/15 bg-white/5 p-3 outline-none select-none shadow-[0_18px_60px_rgba(15,23,42,0.26)] backdrop-blur-xl [-webkit-tap-highlight-color:transparent] [-webkit-touch-callout:none] [background:var(--st-bg)] [border-radius:var(--st-radius)] data-[swiping]:cursor-grabbing group-data-[dismissible=false]:cursor-default group-data-[dismissible=false]:touch-auto"
            role="status"
            aria-live="polite"
            aria-atomic="true"
            tabIndex={0}
          ...
          >
            ...
            <span className="flex min-w-0 flex-auto flex-col gap-0.5">
              <span className="text-[15px] leading-[1.3] font-medium">{title}</span>
              {description ? (
                <span className="text-[12px] leading-[1.3] font-normal [color:color-mix(in_srgb,var(--st-ink)_70%,transparent)]">
                  {description}
                </span>
              ) : null}
            </span>
            {actionLabel ? (
              <button
                type="button"
                className="h-[30px] flex-none cursor-pointer touch-manipulation rounded-full border border-white/15 bg-white/10 px-3 text-[12px] font-medium text-white shadow-[0_8px_20px_rgba(15,23,42,0.18)] backdrop-blur-md outline-none [-webkit-tap-highlight-color:transparent] [font-family:inherit] [transition:transform_160ms_cubic-bezier(0.23,1,0.32,1),background-color_160ms_ease,opacity_160ms_ease] active:[transform:scale(0.97)] motion-reduce:active:[transform:none] [@media(hover:hover)_and_(pointer:fine)]:hover:bg-white/15"
                onClick={() => {
